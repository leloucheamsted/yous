...
class HtmlInjector {
  ...
  final List<String> readiumFonts;
  ...
  Resource transform(Resource resource) => LazyResource(() async {
        ...
        if (link.mediaType.isHtml) {
          return _InjectHtmlResource(publication, viewportWidthGetter,
              userPropertiesPath, customResources, readiumFonts, resource);
        }
        return resource;
      });
}
...
class _InjectHtmlResource extends TransformingResource {
  ...
  String injectReflowableHtml(String content) {
    ...
    endIncludes
        .add(getHtmlLink("/readium/readium-css/ReadiumCSS-pagination.css"));
    endIncludes.add(getHtmlScript("/readium/scripts/readium-reflowable.js"));
    endIncludes.add(getHtmlLink("/readium/fonts/fonts.css"));
  ...
  String applyDirectionAttribute(String resourceHtml, Publication publication) {
    ...
    String addRTLDir(String tagName, String html) =>
        regexForOpeningHTMLTag(tagName).firstMatch(html)?.let((result) {
          var tagHtml = html.substring(result.start, result.end);
          if (tagHtml.toLowerCase().contains(' dir=')) {
            return html;
          }
          var beginHtmlIndex =
  ...
  String injectFixedLayoutHtml(String content) {
    ...
    // }
    includes.add(getHtmlLink("/readium/fonts/fonts.css"));
