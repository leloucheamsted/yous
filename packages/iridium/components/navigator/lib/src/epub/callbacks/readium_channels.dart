...
class ReadiumChannels extends JavascriptChannels {
  ...
  final WebViewPanGestureRecognizer? webViewPanGestureRecognizer;
  ...
  ReadiumChannels(
      ...
      this.webViewPanGestureRecognizer,
      this.listener)
      : locator = Locator(
  ...
  bool _onTap(List<dynamic> arguments) {
    ...
    double clientWidth = event.viewportWidth;
  ...
  void _onSwipeUp(List<dynamic> arguments) {
    // we don't want this, it's weird for user
    // and actually, it's a bit buggy, you can trigger swipe left/right, and swipe up/down at same time
    // causing the page to change and change size at same time, causing unexpected page skip
    ...
    if (arguments.isNotEmpty) {
      if (logArguments) {
        Fimber.d(
            ...
            "${arguments.first.runtimeType}, recognizer: $webViewPanGestureRecognizer");
      }
      bool visibility = arguments.first;
      webViewPanGestureRecognizer?.setLeftOverlayVisible(visibility);
    }
  }
  ...
  void _onRightOverlayVisibilityChanged(List<dynamic> arguments) {
    if (arguments.isNotEmpty) {
      ...
      bool visibility = arguments.first;
      webViewPanGestureRecognizer?.setRightOverlayVisible(visibility);
    }
  }
}
