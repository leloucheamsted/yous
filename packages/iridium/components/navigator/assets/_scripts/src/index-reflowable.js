...
var createPaginator = function() {
    ...
    for (let i = 0; i < nbCols; i++) {
      let divText =
        ...
        "   </div>" +
        "</div>";
        allPaginator += divText;
    }
    paginator.innerHTML = allPaginator;
    //    paginator.show();
...
// when paginator are created on webview that are offscreen in ios
// this can cause a big freeze if there is a lots of pages
// we don't need to do it until it's really necessary = user start trying to interact with the page
var canCreatePaginator = false;
window.addEventListener("load", (event) => {
  document.addEventListener('touchstart', function() {
    if (canCreatePaginator == false) {
      createPaginator();
    }  
    canCreatePaginator = true;
  }, { once: true });
});

readium.initPagination = function () {
  document.fonts.ready.then(async function () {
    if (canCreatePaginator == true) {
      createPaginator();
    }
  });
};
